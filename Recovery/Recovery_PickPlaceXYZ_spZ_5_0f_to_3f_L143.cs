// Auto-generated by Program_TestRecovery.
// This class is compiled at runtime and loaded after Factory I/O is paused.

namespace Controllers
{
    public sealed class Recovery_PickPlaceXYZ_spZ_5_0f_to_3f_L143 : Recovery_PickPlaceXYZ
    {
        public Recovery_PickPlaceXYZ_spZ_5_0f_to_3f_L143()
            : base(new PickPlaceXYZRecoveryDecision
            {
                PickingState = State.State0,
                GrabState = State.State0,
                Counter = 0,
                ExitBox = 0,
                StopExitBox = 1,
                StateIdentificationSatisfied = false,
                RecoveryModule = RecoveryModule.MisalignmentPallet,
                GrabReleaseOperations = new[]
                {
                    new PickPlaceXYZGrabReleaseOperation(3.1f, 5.9f, 5f, 3.1f, 5.3f, 5f, true, true),
                },
                Reason = @"Stage 1 state identification skipped for label 'misalignment_pallet'; defer to recovery module identification. Stage 2 recovery module: misalignment_pallet module selected. Misalignment pallet recovery will run 1 configured grab-release operations before benign resume."
            })
        {
        }

        // Paused snapshot used to make the decision:
        // partAtPlace=False, boxAtPlace=True, detected=False
        // partConveyorForward=False, boxConveyorForward=True, exitConveyor=False
        // grab=False, c=True
        // spX=3.1, spY=5.3, spZ=0
        // posX=3.1001012, posY=5.3000793, posZ=0
        // counter=0, exitBox=1
    }
}
