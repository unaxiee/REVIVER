...
{
    public class Recovery_PickPlaceXYZ_spX_3_1f_to_2_0f_L124 : Controller
    {
        ...
        public Recovery_PickPlaceXYZ_spX_3_1f_to_2_0f_L124()
        {
        ...
        {
            if (grabState == State.State4)
            {
                spZ.Value = 5f;
                if (detected.Value)
                {
                    spZ.Value = posZ.Value;
                    grabState = State.State5;
                }
            }
            else if (grabState == State.State5)
            {
                grab.Value = true;
                ...
                {
                    grabTimer.IN = false;
                    grabState = State.State6;
                }
            }
            else if (grabState == State.State6)
            {
                spZ.Value = 0f;
                if (Near(spZ.Value, posZ.Value, 0.01f))
                {
                    grabState = State.State7;
                }
            }
            else if (grabState == State.State7)
            {
                spX.Value = 3.1f;
                if (Near(spX.Value, posX.Value, 0.01f))
                {
                    grabState = State.State8;
                }
            }
            else if (grabState == State.State8)
            {
                spZ.Value = 5f;
                if (Near(spZ.Value, posZ.Value, 0.01f))
                {
                    grab.Value = false;
                    grabState = State.State9;
                }
            }
            else if (grabState == State.State9)
            {
                spZ.Value = 0;
                if (Near(spZ.Value, posZ.Value, 0.01f))
                {
