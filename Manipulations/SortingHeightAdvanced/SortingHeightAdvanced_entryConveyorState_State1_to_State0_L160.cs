...
{
    public class SortingHeightAdvanced_entryConveyorState_State1_to_State0_L160 : Controller
    {
        ...
        //MemoryBit emitter = MemoryMap.Instance.GetBit("Emitter 1 (Emit)", MemoryType.Output);
        ...
        public SortingHeightAdvanced_entryConveyorState_State1_to_State0_L160()
        {
            ...
            if (turnTableState == State.State0)
            {
                if (atTurntableEntry.Value)
                {
                    if (partsFIFO.Count > 0)
                    {
                        ...
                        {
                            //High boxes
                            turnTableState = State.State10; 
                            entryConveyorState = State.State1;
                        }
                        else
                        {
                            //Low boxes
                            turnTableState = State.State20; 
                            entryConveyorState = State.State0;
                        }
                    }
                }
            }
            else if (turnTableState == State.State10)
            {
                load.Value = true;
